...
const jwt = require("jsonwebtoken"); //importing jsonwebtoken for user token authentication
...
//Authentication with Token middleware function
  ...
  if (jwtToken === undefined) {
    ...
    jwt.verify(jwtToken, "MY_SECRET_TOKEN", async (error, payload) => {
      if (error) {
        ...
        next(); // After successful verification of token proceed to next middleware or handler
      }
    });
  }
}
...
app.post("/login/", async (request, response) => {
  ...
  if (databaseUser === undefined) {
    ...
    if (isPasswordMatched === true) {
      ...
      response.send({ jwtToken }); //Successful login of the user
    } else {
      response.status(400);
      response.send("Invalid password"); //If the user provides an incorrect password
    }
  }
});

//getting  list of all states in the state table
